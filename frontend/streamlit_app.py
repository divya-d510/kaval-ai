...
with st.sidebar:
    ...
    st.caption(
        "Role selection simulates authentication for this prototype — a real deployment would "
        "wire this to Catalyst's user/session system instead. See README Security Notes."
    )
...
visible = [(label, group) for label, group in TAB_DEFS if ROLE in PERMISSIONS[group]]
tab_objs = st.tabs([label for label, _ in visible])
TABS = dict(zip((g for _, g in visible), tab_objs))
...
if "sociological" in TABS:
    with TABS["sociological"]:
...
if "decision_support" in TABS:
    with TABS["decision_support"]:
